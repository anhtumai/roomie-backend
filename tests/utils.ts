...
async function registerUser(api: SuperTest<supertest.Test>, user: any): Promise<void> {
  await api
    .post('/api/accounts')
    .send(user)
    .expect(201)
...
async function registerUsers(api: SuperTest<supertest.Test>, users: any[]): Promise<void> {
...
async function login(
  ...
  const response = await api.post('/api/auth/login').send({ username, password }).expect(200)
